...
async fn buy_item_tx(
    ...
    // Transferir el NFT a la cuenta del mercado / ponerlo disponible para venta
    ...
    // transferir tokens a la cuenta del mercado
    if transfer_tokens(tx_id, &ft_id, new_owner, &exec::program_id(), price)
        .await
        .is_err()
    {
        // si hay un fallo durante la transferencia del token
        // volvemos a transferir NFT al vendedor
    ...
    // Enviar tokens a la cuenta del vendedor y tesoreria
    // como los tokens están en la cuenta del marketplace, el error sólo puede ser debido a la falta de gas
