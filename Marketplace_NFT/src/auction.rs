...
pub trait AuctionHandler {
    ...
    ///
    /// Argumentos:
    /// * `nft_contract_id`: la dirección del contrato NFT.
    /// * `token_id`: el id de NFT
    ///
    /// En caso de éxito de la subasta responde [`MarketEvent::AuctionSettled`].
    /// Si no se han realizado pujas responde [`MarketEvent::AuctionCancelled`].
            ...
            if let Some((tx_id, tx)) = item.tx.clone() {
                match tx {
                    ...
                    MarketTx::SettleAuction => {
                        ...
                        // calcular la tasa de ganancia / comision
                        let treasury_fee =
                            price * (self.treasury_fee * BASE_PERCENT) as u128 / 10_000u128;

                        // pagos por venta de NFT (incluye cuentas de derechos y vendedor)
                        let mut payouts =
...
async fn settle_auction_tx(
    ...
    // enviar tokens a la cuenta del vendedor y tesorería
    // dado que los tokens están en la cuenta del mercado, el error sólo puede deberse a la falta de gas
